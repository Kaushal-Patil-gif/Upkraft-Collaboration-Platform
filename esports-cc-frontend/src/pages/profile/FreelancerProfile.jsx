...
import { HiMail, HiUser, HiLocationMarker, HiGlobeAlt, HiChatAlt, HiLightningBolt, HiBriefcase } from "react-icons/hi";
import KycStatus from "../../components/profile/KycStatus";
import { getUserProfile } from "../../services/userService";
import { getKycStatus } from "../../services/kycService";
import { getFreelancerServices, deleteService } from "../../services/freelancerService";
...
export default function FreelancerProfile() {
  ...
  const fetchServices = async () => {
    ...
    try {
      const servicesData = await getFreelancerServices();
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-4">
              <div className="text-right">
                ...
                <p className="text-lg font-semibold text-gray-900">{user?.name || 'Freelancer'}</p>
              </div>
              <div className="w-12 h-12 bg-gradient-to-r from-blue-500 to-purple-500 rounded-full flex items-center justify-center">
                <span className="text-white font-bold text-lg">{(user?.name || 'F')[0].toUpperCase()}</span>
              </div>
            </div>
          </div>
        </motion.div>
        ...
        <div className="space-y-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="bg-gradient-to-br from-green-50 to-emerald-50 p-4 rounded-xl border border-green-100">
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <h3 className="text-sm font-semibold text-green-900">Professional Name</h3>
                      </div>
                      <p className="text-gray-900 font-medium">{profileData.professionalName || 'Not set'}</p>
                    </div>
          ...
          {activeTab === "services" && (
            ...
            >
              ...
              {servicesLoading ? (
                <div className="text-center py-8">
                  <div className="w-6 h-6 border-2 border-purple-600 border-t-transparent rounded-full animate-spin mx-auto mb-4"></div>
                  <p className="text-gray-500">Loading services...</p>
                </div>
              ) : services.length > 0 ? (
                <div className="space-y-4">
                  {services.map((service) => (
                    <div key={service.id} className="bg-gradient-to-r from-white to-gray-50 border border-gray-200 rounded-xl p-6 hover:shadow-lg transition-all">
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          ...
                          <p className="text-gray-600 mb-3 whitespace-pre-wrap break-words">{service.description}</p>
                        ...
                        <div className="flex items-center gap-2 ml-4">
                          <motion.button
                            whileHover={{ scale: 1.05 }}
                            whileTap={{ scale: 0.95 }}
                            onClick={() => navigate(`/profile/edit-service/${service.id}`)}
                            className="text-blue-600 hover:text-blue-700 font-medium text-sm px-3 py-1 rounded-lg hover:bg-blue-50 transition-all"
                          >
                            Edit
                          </motion.button>
                          <motion.button
                            ...
                            className="text-red-600 hover:text-red-700 font-medium text-sm px-3 py-1 rounded-lg hover:bg-red-50 transition-all"
                          >
                            Delete
                          </motion.button>
                        </div>
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-gray-500 text-lg font-medium mb-4">No services yet</p>
                  <motion.button
                    whileHover={{ scale: 1.05 }}
                    whileTap={{ scale: 0.95 }}
                    onClick={() => navigate("/profile/add-service")}
